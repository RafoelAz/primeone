...
function adaptive_header(w, h) {
  ...
  var headerLang = $(".top-header-lang");
  ...
  if (w < 769) {
    ...
    if (headerLang.hasClass("done")) {
      headerLang.removeClass("done").prependTo($(".top-header"));
    }
  }

  if (w < 769) {
    if (!$(".bottom-header__nav").hasClass("done")) {
      $(".bottom-header__nav").addClass("done").appendTo(headerMenu);
    }
  } else {
    $.each($(".bottom-header__nav"), function (index, val) {
      if ($(this).hasClass("bottom-header__nav-right")) {
        if ($(this).hasClass("done")) {
          $(this).removeClass("done").prependTo($(".bottom-header-row").eq(2));
        }
      } else {
        if ($(this).hasClass("done")) {
          $(this).removeClass("done").prependTo($(".bottom-header-row").eq(0));
        }
      }
    });
  }
